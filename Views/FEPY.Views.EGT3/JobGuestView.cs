...
namespace FEPV.Views
{
    public partial class JobGuestView : UserControl
    {
        ...
        private void tbMac_TextChanged(object sender, EventArgs e)
        {
            try
            {
                //lookUpEdit1.Text = tbMac.Text;
                //cbCNO.Text = lookUpEdit1.EditValue.ToString();
               // cbCNO.Text = lookUpEdit1.SelectedText; 
            }
            catch(Exception) { }
        }
        ...
        public Dictionary<string, object> Paras
        {
            set
            {
                ...
                tbFilePath.Text = "";//
                lblWorkFlow.Text = "";//
                //Leo-Start: Add ValidTo and CNO when Swipe IDCard
                try
                {
                    Name = value["GuestName"].ToString();//GuestName
                    IDCardName = value["IdCard"].ToString();//Card Name C0001, V0001, .....
                    DataTable dt = rep.GetMISReport("EGT3_GetContractorInfo", new string[] { "Name", "IdCard" }, new object[] {Name, IDCardName }).Tables[0];
                    tbValidTo.Text = dt.Rows[0]["ValidTo"].ToString();
                    cbCNO.Text = dt.Rows[0]["CNO"].ToString();
                    if (DateTime.Compare(Convert.ToDateTime(tbValidTo.Text), DateTime.Now) < 0)
                    {
                        tbValidTo.ForeColor = Color.Red;
                        tbValidTo.Text = dt.Rows[0]["ValidTo"].ToString() + "- expired";
                        IsNeedGetCardBack = true;
                    }
                    else
                    {
                        tbValidTo.ForeColor = Color.Black;
                        IsNeedGetCardBack = false;
                    }               
        ...
        public string _cbCNO
        {
            get { return cbCNO.Text.Trim(); }
            set { cbCNO.Text = value; }
        }
        ...
        public DataTable dtListCardData
        {
            set
            {
                lookUpEdit1.Properties.DataSource = value;
                lookUpEdit1.Properties.DisplayMember = "CardID";
                lookUpEdit1.Properties.ValueMember = "CNO";
            }
        }
